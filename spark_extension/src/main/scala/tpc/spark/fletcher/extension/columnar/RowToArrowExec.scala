...
class RowToArrowExec(override val child: SparkPlan)
  ...
  override def doExecuteColumnar(): RDD[ColumnarBatch] = {
    ...
    child.execute().mapPartitions { rowIter =>
      val allocator =
        SparkArrowUtils.rootAllocator.newChildAllocator(
          s"${this.getClass.getSimpleName}",
          0,
          Long.MaxValue)

      val arrowSchema =
        SparkArrowUtils.toArrowSchema(schema, conf.sessionLocalTimeZone)
