...
import tpc.spark.fletcher.extension.columnar.selection.{
  ColumnarWithSelectionToRowExec,
  ColumnarWithSelectionVectorSupport
}
      ...
      case plan @ ColumnarToRowExec(child) =>
        if (child.isInstanceOf[ColumnarWithSelectionVectorSupport])
          ...
          plan.withNewChildren(
            plan.children.map(p => postColumnarTransitions(p))
          )
      case RowToColumnarExec(child) =>
        new RowToArrowExec(postColumnarTransitions(child))
      case plan =>
