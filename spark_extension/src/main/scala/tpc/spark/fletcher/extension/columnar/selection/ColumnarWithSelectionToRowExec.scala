...
import tpc.spark.fletcher.utils.StartStopMeasurement
...
class ColumnarWithSelectionToRowExec(override val child: SparkPlan)
    extends ColumnarToRowExec(child)
    with StartStopMeasurement {
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    val childWithSelection: ColumnarWithSelectionVectorSupport =
  ...
  override lazy val metrics: Map[String, SQLMetric] = Map(
    ...
    "numInputBatches" -> SQLMetrics.createMetric(sparkContext,
                                                 "number of input batches"),
    "conversionTime" -> SQLMetrics
      .createNanoTimingMetric(sparkContext, "time converting in [ns]")
  )

  private class SelectionVectorRowIterator(val columnarBatch: ColumnarBatch,
                                           val selectionVector: SelectionVector)
