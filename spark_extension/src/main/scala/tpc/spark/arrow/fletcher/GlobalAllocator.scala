...
object GlobalAllocator {
  ...
  private val allocator = new RootAllocator(
    ...
    Long.MaxValue,
    Alignment64BitRoundingPolicy
  )

  def newChildAllocator(
      name: String,
      initReservation: Long = 0
  ): BufferAllocator = {
...
object Alignment64BitRoundingPolicy extends RoundingPolicy {
  ...
  private val memoryCacheAlignment = {
    val field = classOf[PooledByteBufAllocator].getDeclaredField(
  ...
  override def getRoundedSize(requestSize: Long): Long =
    if (requestSize == 0)
      0
    else
      DefaultRoundingPolicy.INSTANCE.getRoundedSize(
        roundUpToMultipleOfChunkSize(requestSize)
      )
