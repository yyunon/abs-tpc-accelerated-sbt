...
//import io.netty.buffer.{ArrowBuf, BufferAllocator}
...
class JavaMemoryPoolServer(val allocator: BufferAllocator) {
  ...
  def allocate(size: Long): Long = {
    ...
    val buffer = allocator.buffer(
      Math.max(BaseAllocator.nextPowerOfTwo(size), minAllocationSize)
    )
    buffers.put(buffer.memoryAddress, buffer)
    //println(
    //  "Allocator called for size " + size
    //    .toString() + "at address " + buffer.memoryAddress().toString()
    //)
  ...
  def reallocate(oldAddr: Long, newSize: Long): Long = {
    ...
    val newBuffer = if (oldCapacity < newSize) {
      ...
      val newCapacity =
        BaseAllocator.nextPowerOfTwo(Math.max(newSize, oldCapacity * 2))
