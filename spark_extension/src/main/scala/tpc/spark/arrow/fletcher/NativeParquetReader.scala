...
class NativeParquetReader(
  ...
  private val log = Logger.getLogger(classOf[NativeParquetReader].getName)
  ...
  override def next(): VectorSchemaRoot = {
    ...
    val res = preLoadedBatch.getOrElse(
  ...
  def readNextBatchIfAvailable(): Option[VectorSchemaRoot] = {
    ...
    val vectors: mutable.Buffer[FieldVector] =
      outputSchema.getFields.asScala.zipWithIndex.map {
        case (field, i) =>
          val validityBuffer: ArrowBuf =
            memoryPool.getBufferByAddress(bufferAddresses(i * 3)).orNull
          val valueBuffer: ArrowBuf = memoryPool
            .getBufferByAddress(bufferAddresses(i * 3 + 1))
            .getOrElse(throw new IllegalArgumentException())
          val offsetBuffer: ArrowBuf =
            memoryPool.getBufferByAddress(bufferAddresses(i * 3 + 2)).orNull

          val fieldNode =
            new ArrowFieldNode(resultLengths(i), resultNullCounts(i))

          val vector = field.createVector(allocator)

          vector match {
            case b: BaseFixedWidthVector =>
              vector.loadFieldBuffers(
                fieldNode,
                List(validityBuffer, valueBuffer).asJava
              )
            case b: BaseVariableWidthVector =>
              vector.loadFieldBuffers(
                fieldNode,
                List(validityBuffer, valueBuffer, offsetBuffer).asJava
              )
            case _ =>
              throw new IllegalArgumentException(
                s"${field.getFieldType} not supported."
              )
          }
  ...
  @native def initNativeParquetReader(
  ...
  @native private def readNext(
      ptr: Long,
      lengths: Array[Long],
      nullCounts: Array[Long],
      bufAddrs: Array[Long]
  ): Boolean;
